...
pathPlanner::pathPlanner(std::string path)
{
    ...
    cameFrom.resize(map.rows,std::vector<ppPair>(map.cols,{0,0}));
...
std::deque<ppPair> pathPlanner::BFSPlan(ppPair start, ppPair goal)
{
    BFS(start,goal);
    return getPath(start,goal);
}

std::deque<ppPair> pathPlanner::GBFSPlan(ppPair start, ppPair goal)
{
    GBFS(start,goal);
    return getPath(start,goal);
}

std::deque<ppPair> pathPlanner::AStarPlan(ppPair start, ppPair goal)
{
    AStar(start,goal);
    return getPath(start,goal);
}
...
void pathPlanner::brushfirePoint(ppPair start)
{
    ...
    std::queue<ppPair> frontier;
    frontier.push(start);

    while (!frontier.empty())
    {

        ppPair current = frontier.front();
        frontier.pop();
...
//Planning Algorithms
void pathPlanner::pairToNode(ppPair var1, node &var2)
{
    var2.x = var1.x;
    var2.y = var1.y;
}

void pathPlanner::BFS(ppPair start, ppPair goal)
{
...
void pathPlanner::GBFS(ppPair startPair, ppPair goalPair)
{
...
void pathPlanner::AStar(ppPair startPair, ppPair goalPair)
{
    for (int i = 0; i < map.rows; i++)
    {
        for (int j = 0; j < map.cols; j++)
...
std::deque<ppPair> pathPlanner::getPath(ppPair start, ppPair goal)
{
    ppPair current = goal;
    std::deque<ppPair> path;
