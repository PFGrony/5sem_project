...
void mapPlanning::planPath(vector<coordinate> conVec)
{
	...
	for (int i = 0; i < conVec.size(); i++)
	{
		...
		{
            if (mapWithPaths.at<cv::Vec3b>(path.at(i).y, path.at(i).x) == cv::Vec3b(0, 0, 255))
                mapWithPaths.at<cv::Vec3b>(path.at(i).y, path.at(i).x) = cv::Vec3b(255, 100, 0);
            else if (mapWithPaths.at<cv::Vec3b>(path.at(i).y, path.at(i).x) == cv::Vec3b(255, 100, 0))
                mapWithPaths.at<cv::Vec3b>(path.at(i).y, path.at(i).x) = cv::Vec3b(255, 0, 165);
            else
